...
function createCluster() {
    ...
    cluster.on("exit", (worker, code, signal) => {
        console.log(`Worker PID ${worker.process.pid} died (code: ${code}, signal: ${signal}) in cluster`);
...
async function masterProcessServer(req, res, workerIds) {
        ...
        Promise.race(workerPromise)
            ...
            if (message.type === "entry" || message.type === "api_1") {
                ...
                if (message.body.data) {
                    let data = `This is worker intercommunication api . Your message: ${message.body.data} with total number of request handled ${message.requestCount} by the current worker ${message.workerPID} which is running on port ${workerPorts[message.workerId]}. \n Your message has been delivered to worker with id ${message.targetWorkerPID} port listening on ${workerPorts[message.targetWorker.id]}.\n Please hit the url http://localhost:${workerPorts[message.targetWorker.id]}/${message.type}   `;
                    // console.log(message)
                ...
                console.log(`Server with id ${message.body.pid} is shutting down gracefully`);
                // res.writeHead(200, message.headers)
                res.writeHead(200, { "Content-Type": "text/plain" });
                message.worker.process.kill("SIGTERM");
                return res.end("Server is shutting down and will respawn automatically in 10sec");
            }
            else {
