...
export function processManager() {
    ...
    if (msg.type === "spawn_worker") {
      ...
      const server = app.listen(port, () => {
        ...
        if (typeof address === "string") {
          console.log(
            `Worker PID ${process.pid} started on port http://localhost:${port} where typeof is string`
          )
        } else if (address && address.port) {
          console.log(
            `Worker PID ${process.pid} started on port http://localhost:${port} where typeof is object`
          )
          if (process.send) {
      ...
      process.on("SIGTERM", (msg) => {
        console.log(
          `worker with PID ${process.pid} is shutting down gracefully`
        )
        server.close(() => {
  ...
  app.use((req, res, next) => {
    requestCount++
    if (process.send) process.send({ type: "no_of_requests", requestCount })
    next()
  })
  // const server = app.listen(port, () => {
  //   console.log(
