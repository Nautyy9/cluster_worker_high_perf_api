...
function createCluster() {
  ...
  async function wait(interval: number): Promise<NodeJS.Timeout> {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.url === "/shutdown") {
      ...
      return res.end(
        `Current Worker with id:  ${workerId} has been shutting down gracefully`
      )
    }
    ...
    const finished = (message: { [arg: string]: any }) => {
      ...
      if (message.type === "worker_comm") {
        ...
        if (targetWorker) {
          ...
          } by the current worker ${workerPID} which is running on port ${
            workerPorts[message.workerId]
          }. \n Your message has been delivered to worker with id ${targetWorkerPID} port listening on ${
            workerPorts[targetWorker!.id]
  ...
  cluster.on("exit", (worker, code, signal) => {
    console.log(
      `Worker PID ${worker.process.pid} died (code: ${code}, signal: ${signal})`,
      "in cluster",
      code
    )
